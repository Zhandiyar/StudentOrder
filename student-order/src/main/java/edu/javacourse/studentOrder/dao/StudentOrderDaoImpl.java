...
public class StudentOrderDaoImpl implements StudentOrderDao {
    ...
    private static final String SELECT_ORDERS =
            "SELECT so.*, ro.r_office_area_id,ro.r_office_name,\n" +
                    ...
                    "INNER JOIN jc_passport_office po_w on po_h.p_office_id = so.w_passport_office_id\n" +
    ...
    private void setParamsForAddress(PreparedStatement stmt, int start, Person person) throws SQLException {
        ...
        stmt.setString(start + 4, h_address.getAppartment());
    }
...
//        return getStudentOrdersTwoSelect();
    ...
    private List<StudentOrder> getStudentOrdersOneSelect() throws DaoException {
        ...
        try (Connection con = getConnection();
            ...
            Map<Long, StudentOrder> maps = new HashMap<>();// создали проверяльщик студенческой заявки
           ...
           int counter = 0;// 0 записей
            while (rs.next()) {
                Long soId = rs.getLong("student_order_id"); //вытащили id студ заявки
                if(!maps.containsKey(soId)){ // у тебя такой студ заявки нет ?
                    StudentOrder so = getFullStudentOrder(rs);

                    result.add(so);
                maps.put(soId,so);
            }
                StudentOrder so = maps.get(soId);// дай мне студ заявку по id
                so.addChild(fillChild(rs));// добавили ребенка
                counter++; //количество записей четко определенно
            }
            if(counter >=  limit){//количество записей >= огрраничения
                result.remove(result.size() - 1); //отбросить
...
//            findChildren(con, result);
    ...
    private StudentOrder getFullStudentOrder(ResultSet rs) throws SQLException {
        StudentOrder so = new StudentOrder();// нет
